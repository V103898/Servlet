...
public class MainServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) {
        try {
            ...
            if (method.equals("GET") && path.matches(MainServlet.POSTS_ID_PATH)) {
                final var id = extractId(path);
                controller.getById(id, resp);
                return;
            }
            if (method.equals("POST") && path.equals(MainServlet.POSTS_PATH)) {
                controller.save(req.getReader(), resp);
                return;
            }
            if (method.equals("DELETE") && path.matches(MainServlet.POSTS_ID_PATH)) {
                final var id = extractId(path);
    ...
    private long extractId(String path) {
        var matcher = MainServlet.ID_PATTERN.matcher(path);
        if (matcher.find()) {
